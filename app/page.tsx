...
export default function Home() {
  function sendQuote(event: FormEvent<HTMLFormElement>) {
    ...
    const body = `
Nouvelle demande de devis - SALIGNAT TRANSPORT

Nom / Société : ${data.get("name")}
Téléphone : ${data.get("phone")}
Lieu de départ : ${data.get("from")}
Lieu d'arrivée : ${data.get("to")}
Détails : ${data.get("details")}
`;

    window.location.href = `mailto:${email}?subject=${encodeURIComponent("Demande de devis - Salignat Transport")}&body=${encodeURIComponent(body)}`;
  }
  ...
  return (
    <main className="min-h-screen bg-[#fbfaf8] text-neutral-950">
      ...
      <section id="accueil" className="relative flex min-h-[86vh] items-center justify-center overflow-hidden px-6 pb-20 pt-28 text-center">
        ...
        <div className="relative z-10 mx-auto max-w-5xl animate-[fadeUp_0.8s_ease-out]">
      ...
      <section className="bg-gradient-to-r from-orange-600 via-orange-500 to-orange-400 px-6 py-8 text-white">
        <div className="mx-auto grid max-w-6xl gap-8 md:grid-cols-2">
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              <h3 className="text-xl font-black">Devis en moins de 24h maximum</h3>
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-6">
            {services.map(([icon, title, text]) => (
              <div
                key={title}
                className="group rounded-2xl border border-neutral-200 bg-[#fbfaf8] p-6 text-center shadow-sm transition duration-300 hover:-translate-y-2 hover:scale-105 hover:border-orange-300 hover:shadow-2xl"
              >
      ...
      <section id="devis" className="bg-[#fafaf8] px-6 py-24">
        <form onSubmit={sendQuote} className="mx-auto max-w-4xl rounded-[2rem] bg-neutral-950 p-8 text-white shadow-2xl md:p-12">
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-2">
            <input name="name" required className="rounded-2xl border border-white/20 bg-white/10 px-5 py-4 outline-none transition focus:border-orange-500" placeholder="Nom / Société" />
            <input name="phone" required className="rounded-2xl border border-white/20 bg-white/10 px-5 py-4 outline-none transition focus:border-orange-500" placeholder="Téléphone" />
            <input name="from" required className="rounded-2xl border border-white/20 bg-white/10 px-5 py-4 outline-none transition focus:border-orange-500" placeholder="Lieu de départ" />
            <input name="to" required className="rounded-2xl border border-white/20 bg-white/10 px-5 py-4 outline-none transition focus:border-orange-500" placeholder="Lieu d’arrivée" />
            <textarea name="details" required className="min-h-32 rounded-2xl border border-white/20 bg-white/10 px-5 py-4 outline-none transition focus:border-orange-500 md:col-span-2" placeholder="Marchandise, date, urgence, détails..." />
          </div>

          <div className="mt-6 flex flex-col gap-4 sm:flex-row">
            <button type="submit" className="rounded-full bg-orange-500 px-8 py-4 font-bold text-white transition hover:scale-105 hover:bg-orange-600">
              Envoyer ma demande
            </button>
            <a href={`https://wa.me/${whatsapp}`} className="inline-flex items-center justify-center gap-2 rounded-full border border-white/30 px-8 py-4 font-bold text-white transition hover:scale-105 hover:border-orange-500 hover:text-orange-500">
              <Icon name="whatsapp" className="h-5 w-5" />
              WhatsApp
            </a>
          </div>
      ...
      <footer id="contact" className="bg-neutral-950 px-6 py-10 text-white">
        <div className="mx-auto grid max-w-7xl gap-8 md:grid-cols-4 md:items-center">
          ...
          <a href={`tel:${phone}`} className="flex items-center gap-3 hover:text-orange-500">
            <Icon name="phone" className="h-5 w-5" />
            06 52 82 71 48
          </a>

          <a href={`mailto:${email}`} className="flex items-center gap-3 hover:text-orange-500">
          ...
          <p className="flex items-center gap-3">
            ...
            481 Route de Riottier, 69400 Villefranche-sur-Saône
          </p>
        </div>
        ...
        <div className="mx-auto mt-6 max-w-7xl text-center text-sm text-neutral-500">
          ...
          <p className="mt-2">Mentions légales : SALIGNAT TRANSPORT, siège social 481 Route de Riottier, 69400 Villefranche-sur-Saône. Contact : {email}</p>
        </div>
      </footer>
